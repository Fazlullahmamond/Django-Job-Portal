...
def jobseeker_profile(request):
    ...
    if request.method == 'POST':
        ...
        if profile_form.is_valid() and resume_form.is_valid():
            profile_form.save()
            resume_form.save()
            messages.success(request, 'ستاسو پروفایل اپدیت شوی دی')
...
def request_job(request, id):
    if request.user.requests.filter(job=id):
        messages.warning(
            request,
            ' تاسو لا دمخه د دې اعلان لپاره درخواست کړی دی'
        )
        return redirect('home-page')
    if not request.user.profile.resume:
        messages.info(request, 'لومړی، خپل سی وی اپلود کړئ')
        return redirect('jobseeker-profile')
    ...
    messages.success(request, 'ستاسو غوښتنه ولیږل شوه')
    return redirect('home-page')
...
def jobseeker_requests(request):
    ...
    if not requests:
        messages.info(request, 'تاسو تراوسه غوښتنه نه ده لیږلې')
...
def save_job(request, id):
    if request.user.saved_jobs.filter(job=id):
        messages.warning(request, 'دا اعلان لا دمخه خوندي شوی دی')
        return redirect('home-page')
    job = get_object_or_404(Job, id=id)
    save_job = JobSeekerSaveJob(job=job, saved_job=request.user)
    save_job.save()
    messages.success(request, 'اعلان خوندي شو')
    return redirect('home-page')
